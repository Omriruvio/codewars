// kata - https://www.codewars.com/kata/56a32dd6e4f4748cc3000006
...
function getRainfall(town, str) {
  const regex = new RegExp(`(?<=.*${town}:).*`, 'ig');
  let townInfo = str.match(regex);
  if (townInfo) {
    return (townInfo = townInfo[0].match(/\d+\.?\d+?/g));
  } else {
    return -1;
  }
}
function mean(town, str) {
  const townInfo = getRainfall(town, str);
  return townInfo === -1 ? -1 : townInfo.reduce((acc, x) => +x + acc, 0) / townInfo.length;
}
function variance(town, str) {
  const rainFall = getRainfall(town, str);
  const townMean = mean(town, str);
  const variability = rainFall.map((x) => Math.abs(townMean - x) ** 2).reduce((a, b) => +a + +b, 0) / rainFall.length;
  if (townMean !== -1 && rainFall !== -1) {
    return variability;
  } else return -1;
}
